import Al13N_335.RooTUniverse as RU # RooTUniverse Mount
import Al13N_335.Exceptions   as Exceptions # For Handling Exceptions
import Al13N_335.VirtualPath  as RD # VirtualDIR   Mount
import Al13N_335.Graphical    as RG # Graphics
import Al13N_335.RegeX        as RX # RegeX Operations
...
class Interpreter:
    '''
    * Al13N_335.Main.Interpreter.__init__(Configure_Heads=True/False,FlowIntoLoop=True/False,FailSafe=True/False)
    / Configured Everything Inside Of Interpreter()
    / If Configure_Heads Is True And Al13N_335.Main.Interpreter._AddHeads_ Will Be Triggered
    / If FlowIntoLoop Is True Than Go Into Al13N_335.Main.Interpreter._Loop0Start_(FailSafe=FailSafe)
    '''
    ...
    #########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._AddHeads_()
    / Takes No Inputs: Works inside of the Al13N_335.Graphical.Messages Functions
    '''
    def _AddHeads_(self):
        self.IG._AppendHeader_('Interpreter-3.3.5','{F.G}{S.B}>> Interpreter [ Author :: {F.B}J{F.R}4{F.B}ck{F.R}3{F.B}lS{F.M}y{F.B}N{F.G} ]{S.N}')
    ...
    #########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._Logging_(str,str,source=None/str)
        ...
        if str(Target) in self.Logs:
            if str(Handle) in ['s','S','switch','Switch','SWITCH']:
                ...
                if self.Logs[Target][0] == True:
                    ...
                    raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._Logging_','K',str(Target)+str(' Is Not Configured')))
            else:
                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._Logging_','K',str(Handle)+str(' Is Not A Valid Handle')))
        else:
            raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._Logging_','K',str(Target)+str(' Target Does Not Exist')))
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._ToScreen_(str)
    / For Sending Messages To The Screen :: Al13N_335.Graphical.Messages.Display_String
    '''
    ...
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._FetchIO_()
    ...
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._Loop0Close_()
    / Sends Some Logs And Exits Via sys.exit(1)
    '''
    def _Loop0Close_(self):
        self._ToScreen_('@Interpreter-3.3.5 @Closing-Message')
        self._ToScreen_('@Interpreter-3.3.5 \t '+str(self.Logs))
        self._ToScreen_('@Interpreter-3.3.5 \t '+str(self.IR.Universe))
        self.Status = False
        sys.exit(1)
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._HandleIO_(str)
    ...
    def _HandleIO_(self,Command):
        if len(Command) == 0:
            ...
            if str(Command) in self.Link:
                Command = self.Link[str(Command)]
                        ...
                        if str(Stem) == str('*'):
                            ...
                            for BiT in self.IR.Universe['Main'][str(Stem)]:
                                for SyS in self.IR.Universe['Main'][str(Stem)][str(BiT)]:
                                    ...
                                    Message_2 = '@Interpreter-Out\t'+str(BiT)+str('{F.B}\t<->\t{F.W}')+str(SyS)+str('{F.M}\t//\t{F.G}')+str(VaL)
                                    self._ToScreen_(str(Message_2))
                    elif str(RooT) in ['$link','$Link','$LINK']:
                        if str('</>') in str(Stem):
                            Stem = Stem.split('</>')
                            if len(Stem) == 2:
                                ID = str(Stem[0])
                                CM = str(Stem[1])
                                self.Link[str(ID)]=str(CM)
                            else:
                                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._HandleIO_','V',str(Stem)+str(' Expected 2 Entities Got :')+str(len(Stem))))
                        else:
                            raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._HandleIO_','V',str(Stem)+str(' Expected Seperator "</>"')))
            elif str('||') not in Command:
                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._HandleIO_','V',str(Command)+str(' Expected A Seperator Got Nothing')))
            else:
                self._TriggerOp_(str(Command))
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._TriggerOp_(Command)
    / For Level 1 Operations (Al13N 3.3.5)
    '''
    def _TriggerOp_(self,Command):
        if str('||') in str(Command):
            ...
            if Command_Fail == True:
                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','E',str(Command_Reas)))
            else:
                for C in Command_Post:
                    Root = C[0]
                    Tree = C[1:]
                    if str(Root) in [ '+', 'new', 'New', 'NEW' ]: # Capture Al13N_335.RooTUniverse.RooT.NewGalaxy Call
                                ...
                                if str(',') in str(Set):
                                    ...
                                    if len(Set) == 5:
                                        ...
                                        if BitMin.isdigit() == True and BitMin.isdigit() == True and BitPad.isdigit() == True and str(BitTyp) in ['h','b'] and Bitdis.isdigit() == True:
                                            ...
                                            raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Tree[1])+str(' Expected Valid Types int,int,int,str,int')))
                                    else:
                                        raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Tree[1])+str(' Expected 5 Inputs Got ')+str(len(Set))))
                                else:
                                    raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Set)+str(' Expected null Or Seperator ","')))
                        else:
                            raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Tree)+str(' Expected 2 Entities For Generation Got ')+str(len(Tree))))
                            ...
                            if str('/') in str(Value):
                                ...
                                if str(':-') in str(Value):
                                    ...
                                    if self.IR._ValidateBitLocation_(Value) == True and self.IR._CaptureBitType_(Value) != None:
                                        ...
                                        raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Value)+str(' Got A Variable Catch But Non Existant Or Configured')))
                                else:
                                    raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','V',str(Value)+str(' Got A Variable Catch But No Seperator')))
                            try:
                                ...
                                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','E',str(e)+str(' From Operation '+str(C))))
                        ...
                        if str(':-') in str(Target) and str(':-') in str(Output):
                            ...
                            try:
                                ...
                                self._ToScreen_('@Interpreter-3.3.5 '+str(Signal))
                            except Exception as e:
                                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._TriggerOp_','E',str(e)+str(' From Operation ')+str(C)))
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._Loop0Start_(FaleSafe=True/False)
    ...
    def _Loop0Start_(self,FailSafe=False):
        self.Status = True
        try:
            self._ToScreen_('@Interpreter-3.3.5 {F.B}<.>{F.M} '+str(time.asctime()))
            ...
            self._ToScreen_(str('@Interpreter-3.3.5 ')+str(E))
            if FailSafe != True:
                self._Loop0Close_()
        if FailSafe == True:
    ...
    ########################################################################################################################################
    '''
    * Al13N_335.Main.Interpreter._EncodeFile_(PathToFile,Seperator,OutputName,UseHex=True)
    / Used For Encoding A File Under RegeX
    '''
    def _EncodeFile_(PathToFile,Seperator,OutputName,UseHex=True):
        if os.path.isfile(str(PathToFile)) == True:
            Text   = open(str(PathToFile)).read().split('\n')
            Encode = []
            RootRG = RX.RegeX()
            for Line in Text:
                Output = RootRG.ConvertToChar(sstr(Line),str(Seperator))
                if UseHex == True:
                    Output = RootRG.ConvertEncodeIntoHex(Output,str(Seperator))
            ...
            try:
                ...
                raise Exception(Exceptions.RaiseToUser('Al13N_335.Main.Interpreter._EncodeFile_','E',str(E)))
###########################################################################################################################################
