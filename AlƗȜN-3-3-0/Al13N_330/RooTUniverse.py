...
import Al13N_330.Graphical  as Graphics
import subprocess
'''
> RooT()
# Usage: import Al13N_330.RooTUniverse
#        Root = Al13N_330.RooTUniverse.RooT()
>> __init__(self)
->  self.Universe['Main'/'SymL'/'Conf{'Min'/'Max'/'Pad'/'Hex'/'Bin'/'TyP','DiS'}']
->  self.BiTMaP
->  self.Display (Al13N_330.Graphical.Messages)
->>  Added Heads Via (Al13N_330.Graphical.Messages._AppendHeader_)
>> NewGalaxy(self,str)
...
class RooT:
    ...
    #########################################################################################################################################
    '''
    Al13N_330.RooTUniverse.NewGalaxy(str) -> Generates A Galaxy Inside Of self.Universe['Main']
        ...
        if type(Source) is str and type(Max) is int and type(Pad) is int and type(Hex) is str and type(Bin) is str and type(TyP) is str:
            if str(Source) not in self.Universe['Main']:
                ...
                if int(Min) < int(Max):
                    ...
                    raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.NewGalaxy','V','Got '+str(Min)+str('/')+str(Max)+'Expected Min<Max'))
            else:
                raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.NewGalaxy','K','Got '+str(Source)+' Already Exists'))
        else:
            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.NewGalaxy','T','Invalid Arugments Sent'))
    #########################################################################################################################################
    '''
    * Al13N_330.RooTUniverse().Write(list,str,list)
        ...
        if type(Target) is list and type(Type) is str and type(IO) is list:
            if len(Target) == 1:
                if str(Target[0]) not in self.Universe['SymL']:
                    raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','K',str(Target[0])+str(' Got 1 Entity And No Symbolic Link To Represent')))
                else:
                ...
                if str(Type) in ['vE','VE','varextend','VarExtend','VAREXTEND']: # Variable Catch Takes Anything Inside Of IO And Appends As Variables, Takes Anything
                    ...
                    if len(IO) == 1:
                        if type(IO[0]) is str:
                            ...
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(Type)+str(' Expected 1 Enntity As str In ')+str(IO)))
                    else:
                        raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO)+str(' Expected 1 Entity Inside Of IO')))
                    ...
                    if len(IO) == 1:
                        if IO[0] in ['true','True','1',1]:
                            ...
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO[0])+str(' Expected 1 Entity As boolean Type')))
                        self.Universe['Main'][str(Target[0])][str(Target[1])][str(Target[2])] = [
                            'VAR_BOL',
                            IO
                            ]
                    ...
                    if type(IO[0]) is not int:
                        if IO[0].isdigit() == False:
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO[0])+str(' Expected int')))
                        else:
                            IO[0] = int(IO[0])
                    self.Universe['Main'][str(Target[0])][str(Target[1])][str(Target[2])] = [
                        'VAR_INT',
                    ...
                    if len(IO) == 2:
                        if self._ValidateBitLocation_(IO[0]) == True and self._ValidateBitLocation_(IO[1]) == True:
                            ...
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO)+str(' Expected 2 Locations The Are Invalid')))
                    else:
                        raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO)+str(' Expected 2 Entities In IO Got '+str(len(IO)))))
                    ...
                    if len(IO) == 4:
                        if self._ValidateBitLocation_(IO[0]) == True and self._ValidateBitLocation_(IO[1]) == True:
                            ...
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(IO)+str(' Expected 4 Entities In IO Got '+str(len(IO)))))
                        ...
                        if self._ValidateBitLocation_(IO[0]) == True and self._ValidateBitLocation_(IO[1]) == True:
                            if self._ValidateBitLocation_(IO[2]) == True and self._ValidateBitLocation_(IO[3]) == True:
                                if self._ValidateBitLocation_(IO[4]) == True:
                                    ...
                                    raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','K',str(IO[4])+str(' Expected A Valid Location [4]')))
                            else:
                                raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','K',str(IO[3]+str('/')+str(IO[2])+str(' Expected A Valid Location [2][3]'))))
                        else:
                            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','K',str(IO[0])+str('/')+str(IO[1])+str(' Expected A Valid Location [0][1]')))
                    else:
                        raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Write','V',str(len(IO))+str(' Expected 5 Entities in IO ')))
    #########################################################################################################################################
    '''
    * Al13N_330.RooTUniverse().Execute(list,list,IO=None/IO)
        ...
        if type(Target) is list and type(Output) is list:
            ...
            if len(Target) == 1:
                if str(Target) not in self.Universe['SymL']:
                    raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Execute',str(Target[0])+str(' Got 1 Entity And No Symbolic Link To Represent')))
                else:
                    Target = self.Universe['SymL'][str(Target[0])]
            if len(Output) == 1:
                if str(Output) not in self.Universe['SymL']:
                    raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Execute',str(Output[0])+str(' Got 1 Entity And No Symbolic Link To Represent')))
                else:
            ...
            if self._ValidateBitLocation_(Target) == True and self._ValidateBitLocation_(Output) == True:
                ...
                if len(Target_IO) == 0:
                    raise KeyError(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Execute','V',str(Target)+str(' Cannot Execute A Non Configured Location')))
                else:
                    ...
                    if str(Target_IO[0]) in [ # Variables
                        ...
                        self.Universe['Main'][str(Output[0])][str(Output[1])][str(Output[2])] = Out
                        return Out
                    elif str(Target_IO[0]) in [ 'SYS_SHL' ]: # For Executing Shell Commands
                        Operands   = Target_IO[1]
                        Output_T   = subprocess.Popen(Operands,stdout=subprocess.PIPE)
                        Output_O   = Output_T.stdout.read()
                        self.Universe['Main'][str(Output[0])][str(Output[1])][str(Output[2])] = [ 'VAR_EXT', Output_O]
                        return [ 'VAR_EXT', Output_O]
            else:
                raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT.Execute','V',str(Target)+str('/')+str(Output)+str(' Expected Valid Locations')))
    #########################################################################################################################################
    def _ValidateBitLocation_(self,Location):
        ...
        if type(Location) is list:
            if len(Location) == 3:
                ...
                raise Exception(Exceptions.RaiseToUser('Al13N_330.RooT._ValidateBitLocation_','V',str(Location)+' Expected 3 Entities'))
        else:
            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooT._ValidateBitLocation_','T',str(Location)+' Expected list With 3 Entities'))
    #########################################################################################################################################
    def _CaptureBitType_(self,Location):
        if self._ValidateBitLocation_(Location) == True:
            ...
            raise Exception(Exceptions.RaiseToUser('Al13N_330.RooTUniverse.RooT._CaptureBitType_','K',str(Location)+str(' Expected A Valid Location')))
    #########################################################################################################################################
