...
try:
    ...
    if Failed == True:
        raise Exception(Exceptions.RaiseToUser('Al13N_330.Graphical','I',str(Reason)+' Install Colorama\n\t:: pip3 install colorama'))
    else:
        pass
                    ...
                    if len(Line) > 0:
                        if str(Line[:2]) == str('<<') and str('@') in str(Line) and str('>>') in str(Line):
                            ...
                            if CLne != None:
                                ...
                                raise Exception(Exceptions.RaiseToUser('Al13N_330.Graphical.Messages._AppendChapter_','E',str('No Chapter Configured')))
                    else:
                        continue
            except Exception as e:
                raise Exception(Exceptions.RaiseToUser('Al13N_330.Graphical.Messages._AppendChapter_','E',str(e)+str(' From Operaion Of File ')+str(PathToFile)))
    #########################################################################################################################################
    def _ReadChapter_(self,Target=None,Header=None):
        # INFO: Use Al13N_330.Graphical.TriggerAutoReset() Prior To Running For Colors Not To Leak 
        if len(self.Index) != 0:
            if Target != None:
